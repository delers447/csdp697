...
#chess file module
import tkinter as tk, numpy as np, math, QueenSolution as qs, chess_use_model
from PIL import ImageTk, Image


#probs = chess_use_model.get_data(game_board)
...
def Chess_grid(canvas, gridcolor):
    ...
    for j in range(GRIDHEIGHT_START, GRID_HEIGHT, GRIDUNIT_SIZE):
        for i in range(GRIDWIDTH_START, GRID_WIDTH, GRIDUNIT_SIZE):
            ...
            canvas.create_rectangle(i, j, i+GRIDUNIT_SIZE, j+GRIDUNIT_SIZE, fill=fill_color, tag = grid_Tags[counter])
            counter += 1
...
# def Heat_map():
#     heatcolors =['#25d911', '#d2d911', '#d92511']
...
def Button_released(mouseX, mouseY, a, tag, gCenters, side_xpos, side_ypos):
    global queentags_count, queenSelectFlag, numofQueensPlaced
    global tempGridIndex, formerGridColor, hintPressed
    ...
    if(InsideGrid and (not sameSquareFlag)): 
        ...
        a.itemconfig(grid_Tags[tempGridIndex], fill=formerGridColor)
        hintPressed = True
...
def Hint(canvas):
    global queensMatrix, grid_Tags, hintPressed, tempGridIndex, formerGridColor
    gridIndex = 0
    gridNum = 8
    tempGridIndex = 0
    #['#25d911', '#d2d911', '#d92511'] #Green, Yellow, Red
    gridOriginalColor = True
    

    
    queenProb = chess_use_model.get_prob(queensMatrix)
    maxProb = 0
    if(hintPressed):
        hintPressed = False
        for i in range(0, gridNum):
            ...
            for j in range(0, gridNum):
                ...
                currentProb = queenProb[i][j]
                if((currentProb > maxProb) and (queensMatrix[i][j] != 1)):
                    maxProb = currentProb
                    tempGridIndex = gridIndex
           
                gridIndex += 1
                
        #print(grid_Tags[tempGridIndex])
        formerGridColor = canvas.itemcget(grid_Tags[tempGridIndex], "fill")
        canvas.itemconfig(grid_Tags[tempGridIndex], fill='#25d911')
        print("=============================")
    else:
        hintPressed = True
        for i in range(0, gridNum):
            for j in range(0, gridNum):
                gridOriginalColor, currentGridFill = Alt_fill(gridOriginalColor)
                canvas.itemconfig(grid_Tags[gridIndex], fill=currentGridFill)
                gridIndex += 1
...
def DisplayMatrix():
    global queensMatrix, queensPositions, lastqueensPositions, grid_Tags
    print("==========================")
    print("GO!\n")
    #print(grid_Tags)
    print(queensMatrix)
...
def NumofQueensCount():
    ...
    return numofQueensCount
    
    
def Reset(a, gridXpos, gridYpos):
    global queen_Tags, queentags_count, numofQueensPlaced, tempGridIndex, formerGridColor
    global queensPositions, queensMatrix, NUM_OF_QUEENS
    queentags_count = 0
    numofQueensPlaced = 0
    queensMatrix.fill(0)
    a.itemconfig(grid_Tags[tempGridIndex], fill=formerGridColor)
...
def Chess_main(canvas):
    ...
    submit_Button.place(x =((SCREENWIDTH/2)+x1*GRIDUNIT_SIZE-5),
                        y = (SCREENHEIGHT - 1.375*GRIDUNIT_SIZE))
    
    canvas.create_text((SCREENWIDTH/2)+x1*GRIDUNIT_SIZE + 15,
    ...
    hint_Button = tk.Button(canvas,
                              ...
                              bg = bottombar_color,
                              bd = '0',
                              command = lambda:Hint(canvas))
    
    hint_Button.place(x =((SCREENWIDTH/2) - 5 -(blueHelp_Image.width()/2)),
                        y = (SCREENHEIGHT - 1.375*GRIDUNIT_SIZE))
    
    canvas.create_text((SCREENWIDTH/2)-5,
                       (SCREENHEIGHT - 1.3125*GRIDUNIT_SIZE)+50, 
    ...
    reset_Button.place(x = ((SCREENWIDTH/2)-x1*GRIDUNIT_SIZE + 5 - redArrow_Image.width()),
                      y = (SCREENHEIGHT - 1.375*GRIDUNIT_SIZE))
    
    canvas.create_text((SCREENWIDTH/2)-x1*GRIDUNIT_SIZE - redArrow_Image.width() + 30,
                       (SCREENHEIGHT - 1.3125*GRIDUNIT_SIZE)+50,
    ...
    close_Button = tk.Button(canvas,
                             ...
                             bg = bottombar_color, 
                             bd = '0', 
                             command = lambda:CloseApp(canvas))
    
    close_Button.place(x = GRIDUNIT_SIZE,
                       y = (SCREENHEIGHT - 1.375*GRIDUNIT_SIZE))
   
    canvas.create_text(GRIDUNIT_SIZE + 23,
                       (SCREENHEIGHT - 1.3125*GRIDUNIT_SIZE)+50, 
